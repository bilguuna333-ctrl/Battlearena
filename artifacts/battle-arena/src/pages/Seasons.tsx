            ...
            {season.topPlayer ? (
              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                ...
                <Link href={`/profile/${season.topPlayer.username}`} className="flex items-center gap-3 group">
                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${season.topPlayer.avatarSeed || season.topPlayer.username}`} alt={season.topPlayer.username} className="w-12 h-12 rounded bg-black/40 border border-white/10 group-hover:border-primary/50 transition-colors" />
                    <div>
