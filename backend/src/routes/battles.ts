...
import { emitToBattle } from "../lib/socket";
...
async function maybeFinishBattle(battleId: string): Promise<void> {
  ...
  // Apply ELO changes for any non-practice battle (ranked + normal/lobby/invite)
  const affectsElo = !isPractice;

  const changes = affectsElo
    ? computePairEloChanges(
  ...
  // Emit real-time battle finished event
  emitToBattle(battleId, "battle_finished", {
    result,
    winnerId,
    p1EloChange: changes.p1,
    p2EloChange: changes.p2,
  });
  ...
  for (const [user, eloChange, isWinner, isDraw] of [
    [p1, changes.p1, winnerId === battle.player1Id, winnerId === null],
    [p2, changes.p2, winnerId === battle.player2Id, winnerId === null],
  ] as const) {
    ...
    // Apply ELO delta to current rating (not absolute from stale snapshot)
    ...
    // All non-practice battles affect ELO (ranked + normal/lobby/invite)
    const newHighestElo = Math.max(user.highestElo, newElo);
  ...
  // Clean up queue entries pointing to this battle so the user is not
  // auto-redirected back into the (now finished) battle next time they
  // open /battle.
  await db
    .delete(matchQueueTable)
    .where(eq(matchQueueTable.pendingBattleId, battleId));

  // Close any lobby that was started into this battle and remove members
  // so users no longer auto-rejoin after the battle is over.
  const startedLobbies = await db
    .select()
    .from(lobbiesTable)
    .where(eq(lobbiesTable.battleId, battleId));
  for (const lob of startedLobbies) {
    await db
      .update(lobbiesTable)
      .set({ state: "closed" })
      .where(eq(lobbiesTable.id, lob.id));
    await db
      .delete(lobbyMembersTable)
      .where(eq(lobbyMembersTable.lobbyId, lob.id));
  }
}
...
router.post(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    // Emit real-time progress update to opponent
    emitToBattle(id, "battle_progress", {
      userId: req.user.id,
      passedCount: result.passedCount,
      totalCount: result.totalCount,
      isFinished: isFinishedAttempt,
    });
...
router.post(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    // Emit real-time chat update to both players
    emitToBattle(id, "chat_message", {
      id: row.id,
      userId: row.userId,
